...
Work in the package can be divided into three general steps:
1. Data preparation
2. Mortality estimation
3. Model selection procedure

### Data preparation
MLZ uses the S4 class system. Data and life history parameters, i.e., von Bertalanffy Linf and K, are stored in a single object of class `MLZ_data`. Entries in the S4 class can be accessed with `@`.
...
Once L_c is identified, `calc_ML()` can be used to mean lengths from records larger than L_c.
...
The analysis can be repeated by considering alternative numbers of change points (years in which mortality changes).
...
In order to avoid local minima in the negative log-likelihood function, the function by default uses a grid
search in order to use starting values for the change points that are close to the maximum likelihood estimates.

The grid search function can also be called seperately using `profile_ML`. This function also serves as a likelihood profile over the change points.
...
compare_models(list(model1, model2, model3))
```
## Outline of other estimators
### MLCR (Mean Length with Catch Rate)
...
### MLeffort (Mean Length with effort)
