...
The Gedamke-Hoenig model (along with the equilibrium Beverton-Holt equation) is derived assuming continuous spawning and recruitment. A similar derivation is available for annual (pulse) recruitment using the `spawn` argument (the default option uses continuous recruitment). 
```r
ML(Goosefish, ncp = 2, spawn = "annual")
```

This may be useful if recruitment is highly seasonal (a single year class) and/or for a comparison with another assessment method that assumes annual recruitment.
...
To use the MLCR estimator (Huynh et al.), a time series of CPUE is needed:
...
For a multispecies analysis (Huynh et al. 2017), seperate `MLZ_data` objects are created for seperate stocks/species and should be stored in a list:
```{r}
data(PRSnapper)
typeof(PRSnapper)
```
